...
|---------|---------|-------------|--------|
| Direct APK | `ekklesia-playstore-key.jks` | GitHub Releases (v1.0.32); legacy `/download/ekklesia-latest.apk` is updated only during the controlled web rollout | `scripts/build-direct.sh` |
...
- `version`: 1.0.32
- Closed Testing release vC61 will be submitted only after the protected merge, GitHub release publication, and successful verification of the published APK/AAB hashes and signing metadata. Availability remains controlled by Google Play review and tester eligibility; no production-track promotion is part of this release.
