...
- [ ] NEA-272f Re-Review Blocker: Commit `112adf5` fixes strict title signing, but still must NOT deploy. Endpoint tests are module-level xfail and mostly negative/no-500; deploy readiness still needs real non-xfail FastAPI/DB tests for positive register-key, ticket create, vote, uniqueness conflicts, wrong key/nullifier pair, duplicate vote, self-vote, and safe GET fields.
