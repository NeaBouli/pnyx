...
## 2026-06-03 — Codex: Recommendation for Βουλή Fetcher/Source Fix

### Short Answer
Codex recommends **Option E: A + strict quality gate + verified-source policy; B only as fallback; C only as temporary backfill generated by the parser; D not acceptable for release polish**.

### Why
- Current vC30 mobile is much better, but a tappable `parliament_url` that leads to HTTP 403 is still bad UX. It is honest wording, but not release-quality.
- Root cause is not only missing PDF extraction. The fetcher accepts navigation/accessibility/menu boilerplate as `summary_long_el` because length >= 200 is enough to pass channels 1/2.
- `scraper.py` parses Parliament Jina tables but currently discards the PDF cells. That is likely where official PDF URLs should be extracted for future bills.
- `source_links.py` already correctly refuses blocked HTML and only trusts PDF links from scraped content. Keep that policy.

### Recommended Implementation Order for CC
1. **Do not build/upload AAB and do not touch Play Console.** APK rebuild only after a real fix and S10 re-test.
2. **Mobile/API source policy:** do not expose/tap `parliament_url` as user-facing official source for PARLIAMENT bills unless it is verified reachable/readable. If only blocked `parliament_url` exists, show a non-clickable sync/status card instead of opening a 403 page.
3. **Fetcher quality gate:** add a reusable `_is_bad_parliament_text()` / `_clean_parliament_text()` in `services/parliament_fetcher.py` and reject boilerplate even if it is >200 chars. Bad text must not overwrite `summary_long_el`.
4. **PDF extraction from Jina Parliament tables:** extend `_parse_parliament_markdown()` in `apps/api/routers/scraper.py` to parse PDF links from the PDF columns (`Κατατεθέντα`, `Ψηφισθέντα`) and persist/derive a verified official PDF URL.
5. **Data model decision:** if no DB field exists for verified official URL, add one via Alembic (preferred) or use a clearly documented transitional marker. Do not hide source URLs inside `summary_long_el` as long-term design.
6. **Backfill current visible Βουλή bills from parser output**, not manual hardcoded C unless used only for emergency verification. Current target bills: `GR-5294`, `GR-5293`, `GR-0490a766`.
7. **Playwright/headless (Option B) only if Jina/list parsing cannot retrieve PDFs/text.** Do not make browser scraping the primary path unless necessary; it is slower and operationally heavier.
8. **Reviewed AI analysis remains NEA-301**, separate from official source/text ingestion. Do not mark `ai_summary_reviewed=true` unless an actual reviewed analysis exists.

### Option Assessment
- **A:** partially correct, but too narrow. Extracting PDFs from existing bad `summary_long_el` will not fix future boilerplate ingestion.
- **B:** useful fallback, not first choice. It adds runtime/dependency complexity and may still face blocking.
- **C:** acceptable only as a temporary generated backfill after parser validation. Manual hardcoding is not a durable fix.
- **D:** not acceptable as final release behavior because users can still tap a 403/empty source page.

### Verification Required
- DB/API:
  - `summary_long_el` must be NULL or real text, never Parliament nav/accessibility/menu boilerplate.
  - `official_source_url` must be a verified reachable PDF/readable official source, not blocked HTML.
- S10:
  - Βουλή detail must show useful summary.
  - If verified official source exists: clickable source opens useful content.
  - If no verified source exists: show non-clickable sync/status message, no 403 link.
  - No vote controls for `PARLIAMENT_VOTED`.
  - No stale unavailable text and no webchrome as official text.
- Tests:
  - `python3 -m py_compile apps/api/services/parliament_fetcher.py apps/api/routers/scraper.py apps/api/services/source_links.py`
  - relevant pytest for source links / scraper parsing if available
  - `cd apps/mobile && npx tsc --noEmit` if mobile source policy changes

### Codex Finding
The phrase “vC30 App-Code is correct” is only partially true. It correctly avoids showing boilerplate and vote controls, but the user-facing fallback link to a known-403 `parliament_url` should be removed or made non-clickable until a verified official source URL exists.
