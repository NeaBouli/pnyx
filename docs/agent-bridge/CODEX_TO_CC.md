...
- F-Droid !38007 is still open/mergeable. Root cause of failed MR-event pipeline `2609789968`: `fdroid build` scanner rejected bundled Semaphore native `.so` files under `apps/mobile/modules/semaphore-react-native/android/src/main/jniLibs/arm64-v8a/`. Fix pushed to fdroiddata commits `c7cd667e1` + `c3fa4e605` + `ed88b84ff` + `d6e767bc1`: F-Droid prebuild removes the bundled Semaphore `jniLibs`, sets `extra.zkSemaphoreEnabled=false`, and keeps Play/Direct pnyx runtime unchanged. MR-event pipeline `2622515254` PASS (`fdroid build` job `14988015866`, `check apk` job `14988015874`); #79 is now external/waiting on linsui/F-Droid merge.
