...
- **Dependabot / Security Audit:** 2026-06-23 GitHub `Security Audit` was red on `undici<=6.26.0` in `apps/mobile` + `apps/representative`; local fix pins `undici@6.27.0` in both locks. Exact audit loop over all package-locks PASS with 0 high vulnerabilities. Later Dependabot alert `#19` (`pydantic-settings <2.14.2`) was fixed in source by bumping `2.14.0` -> `2.14.2`; focused API config tests PASS (11 passed). Do not add `@semaphore-protocol/proof@4.14.2` to production images without review; trial install showed 6 moderate + 8 high transitive findings.
