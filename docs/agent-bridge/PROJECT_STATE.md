...
- **GH#111 Runbook/Test:** `GH111_NULLIFIER_V2_CANARY_RUNBOOK.md` + `scripts/gh111-prepare-nullifier-v2-window.sh` + read-only `gh111_nullifier_v2_canary_check.py` + `gh111_kdf_env_guard.py`; endpoint/evaluator regression proves v2 same-row migration with mocked HLR, Redis in-flight lock, atomic row-locked existing-identity re-registration, before/after canary verdicts, report artifacts, and v2 invariant counters. Runbook now includes one-command no-mutation prep, v2 lifespan probe, retrying health check after the previous pre-HLR 500/readiness abort, and confirmed env-file write/rollback helper.
