...
- **API container:** rebuilt live at `ef7f2c1`; strict Parliament freshness endpoint live; DIAVGEIA `submitted_date` backfill complete (`2441` rows, remaining `0`). `pydantic-settings 2.14.2` verified inside container; `IDENTITY_NULLIFIER_KDF_VERSION=v2`; scoped production ZK enabled for `bill:GR-d4c62ed4`; global rollout flag remains off. Automatic/global ZK rollout code is now guarded to public PARLIAMENT bills only (`ACTIVE`, `WINDOW_24H`, `OPEN_END`); DIAVGEIA, DEMO, hidden, canary, and non-public scopes are rejected by server write paths. ZK Arweave publication remains independently gated by exact `ZK_ARWEAVE_SCOPE_ALLOWLIST` + min group size.
...
- **Web container:** source prepared for vC51 landing badge/hash; live deploy pending after commit.
...
- **S10:** latest hardware smoke remains vC50/v1.0.21. vC51/v1.0.22 is built and artifact-verified, but S10 visual/install retest is pending because no device was attached during the build.
- **Alembic:** `u401a2b3c4d5` (ZK receipt `vote_commitment` NOT NULL, head)
- **Disk:** 2026-06-20 qwen cleanup complete; `llama3.2:3b` remains installed and production-configured. 2026-06-24 disk recheck found `/` back at 93% used / ~5.6 GB free. Hourly `safe-disk-guard.timer` is active, but safe cleanup now reports `cleanup_ineffective` when usage remains high because the pressure is mostly active Docker/containerd data outside auto-clean policy. No volumes/images/backups/data were deleted. Cleanup policy: `docs/agent-bridge/SERVER_CLEANUP_POLICY.md`; guard source: `scripts/safe-disk-guard`; audit/safe-clean helper: `scripts/server-disk-maintenance.sh`.
...
- **F-Droid !38007:** Still open/mergeable. Latest linsui feedback ("Please use sed instead of inline python code") is addressed in fdroiddata commit `2bf46a733`: inline Python prebuild edits were replaced with `sed`/shell commands. MR-event pipeline `2630143862` PASS (`fdroid lint`, `fdroid rewritemeta`, `fdroid build`, `check apk`). #79 waits only on linsui/F-Droid review + merge.
...
- **vC51 Release Prep:** AAB `/Users/gio/Desktop/ekklesia-release-vC51/ekklesia-v1.0.22-vC51-PLAY.aab`, SHA256 `a0176d4597d8da1d2862a66f08aeb84deeaf516287a51ed422dcc2ecadeb45eb`; Direct APK `/Users/gio/Desktop/ekklesia-release-vC51/ekklesia-v1.0.22-vC51-DIRECT.apk`, SHA256 `e83a310d0fa932bdfa53ac87286e6a58bba5a98e9eee0259a142303e74b44b83`.
