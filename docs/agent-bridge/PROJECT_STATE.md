...
- **Disk:** 2026-06-20 qwen cleanup complete; `llama3.2:3b` remains installed and production-configured. 2026-06-23/24 disk recheck found `/var/lib` and Docker Build Cache as the safe cleanup target; ran only `docker builder prune -af` (no volumes/images/backups/data removed). After API/monitor rebuild: `/` 84% used / 12 GB free, containers still up, DB healthy, monitor PASS (18 checks, no alerts). Cleanup policy: `docs/agent-bridge/SERVER_CLEANUP_POLICY.md`; audit/safe-clean helper: `scripts/server-disk-maintenance.sh`.
