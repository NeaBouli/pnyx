...
- **Disk:** 2026-06-20 read-only audit: `/` 89% used / 8.4 GB free, inodes 17%. Main pressure is shared Docker/containerd runtime data, not PostgreSQL row growth or `/opt/ekklesia` app code. Ekklesia-specific high-impact candidate is unused Ollama `qwen2.5:14b` (~9 GB) while production uses `llama3.2:3b`; current code/script defaults no longer depend on qwen. Do not auto-delete models. Cleanup policy: `docs/agent-bridge/SERVER_CLEANUP_POLICY.md`; audit/safe-clean helper: `scripts/server-disk-maintenance.sh`.
