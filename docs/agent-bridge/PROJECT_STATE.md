...
- **GH#111 Backup:** latest no-mutation production preflight package `/opt/ekklesia/backups/pre_gh111_nullifier_v2_canary_20260617_200157`; KDF remains v1, no identity mutation, 17 active / 0 revoked / 0 v2 rows, monitor PASS, isolated v2 lifespan probe PASS, package validation PASS (`ok=true`, no blockers/warnings).
- **GH#111/GH#112 Completion Audit:** `docs/agent-bridge/GH111_GH112_COMPLETION_AUDIT.md` records the current proof boundary: GH#112 first public scoped rollout is complete for `bill:GR-d4c62ed4`; GH#111 is prepared but not complete until real S10/HLR v2 activation + post-verify compare run.
...
- **GH#111 S10 UI Path:** Profile -> `Επαλήθευση / Νέο κλειδί` -> VerifyScreen verified on S10 without submit/HLR; vC50 keeps this path. Fresh 2026-06-17 check: S10 `SM-G973F`, app `versionCode=50`, `versionName=1.0.21`, launch smoke test PASS, no fatal Logcat crash; DB invariant remains 17 active identities, 0 v2 rows, KDF v1/unset, malformed/mismatched v2 counters all 0.
