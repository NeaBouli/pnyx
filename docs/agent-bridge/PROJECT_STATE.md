...
- **Repo HEAD:** siehe `git rev-parse --short HEAD` (latest local work: Parliament-only automatic ZK rollout guard + post-scrape lifecycle catch-up)
- **API container:** rebuilt live at `74ea10f`; `IDENTITY_NULLIFIER_KDF_VERSION=v2`; scoped production ZK enabled for `bill:GR-d4c62ed4`; global rollout flag remains off. Automatic/global ZK rollout code is now guarded to public PARLIAMENT bills only (`ACTIVE`, `WINDOW_24H`, `OPEN_END`); DIAVGEIA, DEMO, hidden, canary, and non-public scopes are rejected by server write paths. ZK Arweave publication remains independently gated by exact `ZK_ARWEAVE_SCOPE_ALLOWLIST` + min group size.
...
- **Monitor container:** rebuilt live at `74ea10f`; latest manual `--once` run PASS (18 checks, no alerts). ZK pending receipts are not escalated while `ZK_ARWEAVE_PUBLICATION_ENABLED=false`; lifecycle stuck monitor has a short grace for newly updated Parliament scrape rows so scrape/lifecycle race noise does not become T3 spam.
...
- **Disk:** 2026-06-20 qwen cleanup complete; `llama3.2:3b` remains installed and production-configured. 2026-06-23 disk alert recheck found `/` at 90% used / 7.5 GB free from Docker Build Cache only; ran `docker builder prune -af` (no volumes/images/backups/data removed). After: `/` 82% used / 14 GB free, Build Cache 0 B, containers still up, DB healthy, monitor PASS (18 checks, no alerts). Cleanup policy: `docs/agent-bridge/SERVER_CLEANUP_POLICY.md`; audit/safe-clean helper: `scripts/server-disk-maintenance.sh`.
...
- **F-Droid !38007:** Still open/mergeable. Failed MR-event pipeline `2609789968` was diagnosed: `fdroid build` scanner rejected bundled Semaphore native `.so` files. Fix pushed in separate fdroiddata branch commits `c7cd667e1` + `c3fa4e605` + `ed88b84ff` + `d6e767bc1`: F-Droid flavor removes bundled Semaphore `jniLibs` before scanner and sets `zkSemaphoreEnabled=false`; Play/Direct pnyx vC50 runtime unchanged. MR-event pipeline `2622515254` PASS (`fdroid build` + `check apk` success). #79 waits only on linsui/F-Droid review + merge.
...
- **Dependabot / Security Audit:** GitHub Dependabot currently reports 0 open alerts. 2026-06-23 GitHub `Security Audit` was red on `undici<=6.26.0` in `apps/mobile` + `apps/representative`; local fix pins `undici@6.27.0` in both locks. Exact audit loop over all package-locks now PASS with 0 high vulnerabilities. Do not add `@semaphore-protocol/proof@4.14.2` to production images without review; trial install showed 6 moderate + 8 high transitive findings.
