...
- **Repo HEAD:** siehe `git rev-parse --short HEAD` (latest local work: ZK Arweave monitor policy follows publication gates)
...
- **Monitor container:** rebuilt live at `dc6db38`; manual `--once` run PASS (17 checks, no alerts). ZK pending receipts are not escalated while `ZK_ARWEAVE_PUBLICATION_ENABLED=false`; if publishing is enabled, monitor only checks exact `ZK_ARWEAVE_SCOPE_ALLOWLIST` scopes and alerts on missing allowlist.
