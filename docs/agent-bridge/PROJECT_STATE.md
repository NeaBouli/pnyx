...
- **F-Droid !38007:** Still open/mergeable. Sed feedback was addressed in fdroiddata commit `2bf46a733` and pipeline `2630143862` passed. Latest linsui ABI feedback is addressed in fdroiddata commit `c23b4b2cd7`: four ABI-specific builds `511-514`, `abiFilters`, `mips/mips64` removal, `VercodeOperation`, `CurrentVersionCode=514`. Local `rewritemeta` + `git diff --check` PASS; new MR pipeline `2634045842` is running. #79 waits on pipeline + linsui/F-Droid review + merge.
