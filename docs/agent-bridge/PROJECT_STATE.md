...
- **F-Droid !38007:** Still open/mergeable. Sed feedback was addressed in fdroiddata commit `2bf46a733` and pipeline `2630143862` passed. Latest linsui ABI feedback is addressed in fdroiddata commits `c23b4b2cd7` + `116cbe6a23` + `f11a3c605e` + `11770592c7`: four ABI-specific builds `511-514`, `abiFilters`, `mips/mips64` removal, `VercodeOperation`, `CurrentVersionCode=514`, CI `rewritemeta` normalization, and final CI-only trailing-space fix. MR pipeline `2634279484` passed (`fdroid lint`, `fdroid rewritemeta`, `fdroid build`, `check apk`). #79 now waits only on linsui/F-Droid review + merge.
