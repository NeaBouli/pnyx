...
- **GH#111 Backup:** latest no-mutation production preflight package `/opt/ekklesia/backups/pre_gh111_nullifier_v2_canary_20260617_064644`; KDF remains v1, no identity mutation, 17 active / 0 revoked / 0 v2 rows, monitor PASS, isolated v2 lifespan probe PASS, remote `package_check.json` PASS (`ok=true`, no blockers/warnings).
- **GH#111 Runbook/Test:** `GH111_NULLIFIER_V2_CANARY_RUNBOOK.md` + `scripts/gh111-prepare-nullifier-v2-window.sh` + guarded activation/rollback helper `scripts/gh111-activate-nullifier-v2-window.sh` + read-only `gh111_nullifier_v2_canary_check.py` + `gh111_kdf_env_guard.py` + `gh111_preflight_package_check.py`; endpoint/evaluator regression proves v2 same-row migration with mocked HLR, Redis in-flight lock, atomic row-locked existing-identity re-registration, before/after canary verdicts, report artifacts, and v2 invariant counters. Runbook now includes one-command no-mutation prep, v2 lifespan probe, retrying health check after the previous pre-HLR 500/readiness abort, confirmed env-file write/rollback helper, mandatory `package_check.json` with `"ok": true`, and emergency rollback attempt if activation fails after KDF write.
