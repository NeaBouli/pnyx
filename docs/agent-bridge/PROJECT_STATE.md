...
- **API container:** rebuilt live at `b34d30d`; `pydantic-settings 2.14.2` verified inside container; `IDENTITY_NULLIFIER_KDF_VERSION=v2`; scoped production ZK enabled for `bill:GR-d4c62ed4`; global rollout flag remains off. Automatic/global ZK rollout code is now guarded to public PARLIAMENT bills only (`ACTIVE`, `WINDOW_24H`, `OPEN_END`); DIAVGEIA, DEMO, hidden, canary, and non-public scopes are rejected by server write paths. ZK Arweave publication remains independently gated by exact `ZK_ARWEAVE_SCOPE_ALLOWLIST` + min group size.
