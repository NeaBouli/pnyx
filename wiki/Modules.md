<!--
@wiki-page MODULES
@update-hint Update on new modules or status changes.
@ai-anchor WIKI_MODULES
-->

# Modules — MOD-01 to MOD-15

| ID | Όνομα / Name | Κατάσταση / Status | Περιγραφή / Description |
|---|---|---|---|
| MOD-01 | Identity | ✅ Beta | SMS HLR, Ed25519, Nullifier, Revocation |
| MOD-02 | VAA | ✅ Beta | VoteCompass, 8 parties, 15 positions, matching |
| MOD-03 | Parliament | ✅ Beta | Βουλή API, Bill Lifecycle (5 States) |
| MOD-04 | CitizenVote | ✅ Beta | Signed voting, vote change |
| MOD-05 | Analytics | ✅ Beta | Divergence Score, k-anonymity |
| MOD-06 | Admin | 🔜 Optional | Data management, moderation queue |
| MOD-07 | Notifications | 🔜 V2 | Push (Expo), WebSocket, RSS |
| MOD-08 | TrueRepublic | 🔜 V2 | Cosmos SDK Bridge, PnyxCoin |
| MOD-09 | Demographics | 🔜 Alpha | gov.gr OAuth2.0, Demographic Hash |
| MOD-10 | Scraper | 🔜 V2 | Crawl4AI, Graceful Fallback |
| MOD-11 | AI Summarizer | 🔜 V2 | Claude API, 3-level summary |
| MOD-12 | Public API | ✅ Beta | REST + Embed Widget + RSS |
| MOD-13 | My MP | 🔜 Beta+ | My representative, score |
| MOD-14 | Relevance | ✅ Beta | Up/Down signal, trending feed |
| MOD-15 | Export | 🔜 Beta | CSV/JSON for NGOs and media |
