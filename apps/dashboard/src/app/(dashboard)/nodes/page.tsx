...
type NodeStatus = 'active' | 'pending' | 'concept' | 'offline'
...
export default function NodesPage() {
  ...
  const [registerForm, setRegisterForm] = useState({ name: '', domain: '', district: '', contact: '' })
    ...
    <div>
      ...
      <div className="space-y-6">
          ...
          <div className="p-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="text-xs text-gray-500 mb-1">Container Status</div>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="text-xs text-gray-500 mb-1">Disk-Verbrauch</div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-medium text-gray-600">Bezirk</th>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5">
          <div className="font-semibold text-blue-800 mb-1">Federation-Architektur</div>
          <div className="text-sm text-blue-700">
            Jeder Bezirk/Kommune kann einen eigenen ekklesia.gr-Node betreiben und sich mit dem
            zentralen Node synchronisieren. Details unter /opt/hetzner-migration/architecture/federation/
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setShowRegisterModal(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="z.B. admin@athens.ekklesia.gr"
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
            </div>
            <div className="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
              ...
              <button
                onClick={() => {
                  alert(`Node "${registerForm.name}" wird in Phase 2 registriert.`)
                  setShowRegisterModal(false)
                  setRegisterForm({ name: '', domain: '', district: '', contact: '' })
                }}
                className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors"
              >
                Registrieren (Phase 2)
              </button>
            </div>
          </div>
        </div>
