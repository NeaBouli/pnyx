...
async def _build_context(question: str, lang: str, db: AsyncSession) -> tuple[str, list]:
...
async def ask_agent(
    ...
    """
    context, bills = await _build_context(req.question, req.lang, db)
    ...
    if _is_answer_poor(ollama_answer):
        ...
        if claude_answer:
            ...
            return {
                ...
                "model": model_used,
                "sources": [{"bill_id": b.id, "title": b.title_el or b.title_en} for b in bills[:5]],
                "lang": req.lang,
            }
